...
import { sessionMessages, sessionPendingClear, sessionPendingList, type RunStats } from "./chat";
...
export function createConverge(deps: {
  ...
  const converge = (
    ...
    void Promise.all([sessionMessages(sid), sessionPendingList(sid)]).then(([messages, q]) => {
