...
export default function McpSection() {
  ...
  const [restarting, setRestarting] = createSignal<ReadonlySet<string>>(new Set());
  ...
  const refreshMcp = async (): Promise<McpServerStatus[]> => {
    const seq = ++refreshSeq;
    try {
      const list = await mcpStatus();
      ...
      if (!disposed && seq === refreshSeq) setLoadErr(errText(error));
      throw error;
    }
  };
  ...
  const restart = async (name: string) => {
    ...
    try {
      ...
      try {
        ...
        flashErr(`重启成功，但刷新状态失败：${errText(error)}`);
      }
    } catch (error) {
      flashErr(`重启失败：${errText(error)}`);
    } finally {
      if (!disposed) {
  ...
  const startMcpAuth = async (name: string) => {
    ...
    const r = await mcpAuth(name).catch((e: unknown) => {
      if (!disposed) setAuthErrs((p) => ({ ...p, [name]: errText(e) }));
      return null;
    });
    if (disposed) return;
    if (!r) {
    ...
    const timer = setInterval(() => {
      void refreshMcp()
        .then((list) => {
          if (disposed) return;
          const cur = list.find((server) => server.name === name);
          if (!cur) return;
  ...
  onMount(() => void refreshMcp().catch(() => undefined));
  ...
  return (
    <div>
      ...
      <Show when={loadErr()}>
        <div class="mb-1.5 text-xs text-[var(--err)]">
          MCP 状态读取失败：{loadErr()}
          <button
            class="ml-2 hover:underline"
            onClick={() => void refreshMcp().catch(() => undefined)}
          >
            重试
          </button>
        </div>
              ...
              <div class="py-1 text-xs">
                <div class="flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={restarting().has(s.name)}
                    onClick={() => void restart(s.name)}
                  >
                    {restarting().has(s.name) ? "重启中…" : "重启"}
                  </button>
                </div>
