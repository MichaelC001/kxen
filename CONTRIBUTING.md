...
4. 按发布矩阵在六个平台的 runner 上并行执行 desktop 与 headless 两类构建。desktop 产出 macOS(arm64、x86_64）的 Developer ID 签名和 Apple 公证 App/DMG、Linux(x86_64、arm64）的 AppImage/deb，以及 Windows(x86_64、arm64）的 NSIS；headless 同时构建 `kxen-<platform>` server 和 `kxen-agent-<platform>` CLI asset，macOS CLI 使用同一 Developer ID 和公证链。两类 artifact 在 publish job 合并后仍作为同一个版本发布，并重新执行完整 asset、updater signature 和 checksum 校验。主线 CI 的 macOS gate 与 release 的 macOS Intel、Windows 较慢路径使用 Rust dependency cache；cache 按组件、toolchain、runner、target、Cargo 配置和 lockfile 隔离，只保存依赖构建结果，不缓存 workspace binary、签名结果或 release asset，并控制在仓库 cache quota 内。发布矩阵的平台、runner、rust target 和稳定 asset 命名以 `scripts/release-manifest.sh` 为单一出处，`scripts/release-manifest.sh json` 可查看实际清单。
