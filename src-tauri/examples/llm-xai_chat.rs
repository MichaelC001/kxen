...
async fn main() {
    ...
    let outcomes = kxen_gui::auth::probe_all(&mut store, true);
    for (p, o, _) in &outcomes {
        eprintln!("probe {p}: {o:?}");
    }

    let config_path = kxen_gui::core::paths::config_dir().join("config.toml");
    let config = match kxen_gui::core::config::Config::load(&config_path, None) {
        Ok(config) => config,
        Err(error) => {
    ...
    let mrm = kxen_gui::llm::mrm::ModelResourceManager::new(config);
    let model = ModelRef::new("xai", "grok-build-0.1");
    let messages = vec![Message::user("Reply with exactly one word: pong")];
    match kxen_gui::llm::managed::collect_text(&mrm, &model, &messages, &store, std::time::Duration::from_secs(60), None, None).await {
        Ok(output) => {
            println!("{}", output.text);
            match output.usage {
