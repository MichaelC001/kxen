...
fn persist_config_at(path: &Path, mutate: impl FnOnce(&mut toml::Table)) -> Result<(), String> {
    ...
    mutate(&mut doc);
    kxen_core::core::config::validate_user_document(&doc, &path.display().to_string()).map_err(|error| error.to_string())?;
...
mod tests {
    ...
    fn persist_config_at_roundtrips_and_keeps_other_keys() {
        ...
        let config = kxen_core::core::config::Config::load(&path, None).expect("reload persisted config");
