...
mod tests {
    ...
    fn run_refresh(base: &str, key: &str, auth_file: &std::path::Path, store: &mut AuthStore, register_path: &str) -> Result<(), String> {
        let rt = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap();
        rt.block_on(run_with_endpoints(
            store,
            key,
    ...
    #[test]
    fn missing_access_token_in_response_fails_after_single_retry() {
        // 200 但缺 accessToken 视为失败：重注册后重试一次，仍缺才把两次错误都抛出来。
        let base = scripted_server(vec![
            ("/token", "HTTP/1.1 200 OK", r#"{"refreshToken":"r2"}"#),
            ("/client/register", "HTTP/1.1 200 OK", r#"{"clientId":"c2","clientSecret":"s2"}"#),
            ("/token", "HTTP/1.1 200 OK", r#"{"refreshToken":"r2"}"#),
        ]);
        let key = format!("test:kiro-no-access-{}", uuid::Uuid::new_v4());
        let auth_file = temp_auth_file("no-access");
        let mut store = AuthStore::new();
        let error = run_refresh(&base, &key, &auth_file, &mut store, "/client/register").expect_err("missing accessToken must fail");
        assert!(error.matches("missing accessToken").count() == 2, "{error}");
        assert!(!store.contains_key(&key));
        std::fs::remove_file(auth_file).ok();
    }

    #[test]
    fn http_error_detail_falls_back_to_error_description() {
        let base = scripted_server(vec![
            ("/token", "HTTP/1.1 400 Bad Request", r#"{"error_description":"refresh token expired"}"#),
            ("/client/register", "HTTP/1.1 500 Internal Server Error", r#"{"message":"boom"}"#),
        ]);
        let key = format!("test:kiro-err-desc-{}", uuid::Uuid::new_v4());
        let auth_file = temp_auth_file("err-desc");
        let mut store = AuthStore::new();
        let error = run_refresh(&base, &key, &auth_file, &mut store, "/client/register").expect_err("http 400 must fail");
        assert!(error.contains("HTTP 400") && error.contains("refresh token expired"), "{error}");
        assert!(!store.contains_key(&key));
        std::fs::remove_file(auth_file).ok();
    }

    #[test]
    fn invalid_json_response_is_an_error() {
        let base = scripted_server(vec![
            ("/token", "HTTP/1.1 200 OK", "not-json{"),
            ("/client/register", "HTTP/1.1 500 Internal Server Error", r#"{"error":"boom"}"#),
        ]);
        let key = format!("test:kiro-badjson-{}", uuid::Uuid::new_v4());
        let auth_file = temp_auth_file("badjson");
        let mut store = AuthStore::new();
        let error = run_refresh(&base, &key, &auth_file, &mut store, "/client/register").expect_err("non-json 200 must fail");
        assert!(error.contains("response was invalid") && error.contains("重新注册"), "{error}");
        assert!(!store.contains_key(&key));
        std::fs::remove_file(auth_file).ok();
    }
}
