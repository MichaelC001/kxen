...
pub(super) async fn handle(method: &str, params: &Value, state: &Arc<AppState>) -> Result<Value, String> {
    match method {
        "knowledge.list" => {
            ...
            serde_json::to_value(kxen_gui::knowledge::list(&dir)).map_err(|error| error.to_string())
        }
        "knowledge.add" => {
            let scope = kxen_gui::knowledge::Scope::parse(params.get("scope").and_then(Value::as_str).unwrap_or("personal"))?;
            ...
            let dir = kxen_gui::core::shared::read(&state.active_workspace).clone();
            let path = kxen_gui::knowledge::add(scope, &dir, slug, kind, description, content)?;
            Ok(json!({ "path": path }))
        }
        "knowledge.remove" => {
            let scope = kxen_gui::knowledge::Scope::parse(params.get("scope").and_then(Value::as_str).ok_or("missing scope")?)?;
            let slug = params.get("slug").and_then(Value::as_str).ok_or("missing slug")?;
            let dir = kxen_gui::core::shared::read(&state.active_workspace).clone();
            kxen_gui::knowledge::remove(scope, &dir, slug)?;
        ...
        "knowledge.set_enabled" => {
            ...
            let dir = kxen_gui::core::shared::read(&state.active_workspace).clone();
            kxen_gui::knowledge::set_enabled(scope, &dir, slug, enabled)?;
            Ok(json!({ "scope": scope.as_str(), "slug": slug, "enabled": enabled }))
        }
        "knowledge.move" => {
            let scope = kxen_gui::knowledge::Scope::parse(params.get("scope").and_then(Value::as_str).ok_or("missing scope")?)?;
            let slug = params.get("slug").and_then(Value::as_str).ok_or("missing slug")?;
            let to = kxen_gui::knowledge::Scope::parse(params.get("to").and_then(Value::as_str).ok_or("missing to")?)?;
            let dir = kxen_gui::core::shared::read(&state.active_workspace).clone();
            let path = kxen_gui::knowledge::move_entry(scope, &dir, slug, to)?;
            Ok(json!({ "path": path }))
        }
        "knowledge.injection_preview" => {
            ...
            let dir = match session_id {
                ...
                None => kxen_gui::core::shared::read(&state.active_workspace).clone(),
            };
            let involved = session_id
                .and_then(|session_id| kxen_gui::core::shared::lock(&state.session_involved).get(session_id).cloned())
                .unwrap_or_default();
            Ok(json!({ "block": kxen_gui::knowledge::render(&dir, &involved) }))
        }
        "knowledge.consolidation_blocked" => {
            serde_json::to_value(kxen_gui::knowledge::consolidate::blocked_attempts()?).map_err(|error| error.to_string())
        }
        "knowledge.consolidation_acknowledge_unknown" => {
            ...
            let result = kxen_gui::knowledge::consolidate::acknowledge_unknown(session_id, &state.session_tokens).await?;
