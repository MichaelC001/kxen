...
pub(super) async fn handle(method: &str, _params: &Value, state: &Arc<AppState>) -> Result<Value, String> {
    match method {
        "workspaces.overview" => {
            let sessions = kxen_gui::core::session::list_checked(&kxen_gui::core::paths::sessions_dir())
            ...
            let workspaces = kxen_gui::core::workspace::list(&kxen_gui::core::paths::data_dir()).map_err(|error| error.to_string())?;
            // queue/cron 都是内存快照，一次锁取出
            let queued = state.pending_messages.counts();
            let cron = kxen_gui::core::schedule::list()?;
            // goal 一次全量读盘后按会话归属分配：逐 session focus_for 会把磁盘读放大 N 倍
            let goals = kxen_gui::core::goal::Goal::list_checked(&kxen_gui::core::paths::goals_dir()).map_err(|error| error.to_string())?;
            ...
            let cards = tokio::task::spawn_blocking(move || {
                kxen_gui::core::workspace::overview(workspaces, &sessions, &running, &queued, &goals, &cron, &worktrees)
            })
            .await
...
async fn gather_worktrees(
    workspaces: &[kxen_gui::core::workspace::Workspace],
) -> Result<std::collections::HashMap<String, Vec<kxen_gui::core::workspace::WorktreeDigest>>, String> {
    ...
    for w in workspaces {
        ...
        set.spawn(async move {
            let list =
                kxen_gui::tools::worktree::list(&root).await.map_err(|error| format!("list worktrees for {}: {error}", root.display()))?;
            let mut out = Vec::with_capacity(list.len());
            for t in list {
                // 单棵树 status 失败以 dirty=None 明确表示 UNKNOWN，不把整张 workspace 卡片降成失败。
                let dirty = kxen_gui::tools::worktree::status(&t.path).await.ok().map(|v| v.len());
                out.push(kxen_gui::core::workspace::WorktreeDigest {
                    name: t.name,
                    branch: t.branch,
...
mod tests {
    ...
    async fn worktree_list_failure_is_not_reported_as_empty() {
        ...
        let workspaces = vec![kxen_gui::core::workspace::Workspace { path: root.to_string_lossy().into_owned(), last_used: 0 }];
    ...
    async fn missing_worktree_store_is_a_valid_empty_result() {
        ...
        let workspaces = vec![kxen_gui::core::workspace::Workspace { path: path.clone(), last_used: 0 }];
