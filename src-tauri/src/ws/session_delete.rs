...
pub(super) async fn delete(params: &Value, state: &Arc<AppState>) -> Result<Value, String> {
    ...
    let lifecycle = kxen_gui::core::session_lifecycle::begin_deletion(id).await?;
    kxen_gui::core::session::load_meta(&sessions_dir, id).map_err(|error| format!("session not found: {error}"))?;
    ...
    let (mut deletion, active) = {
        ...
        let deletion = kxen_gui::core::session_recovery::begin_deletion(&sessions_dir, id)?;
    ...
    while kxen_gui::core::shared::lock(&state.active_runs).contains_key(id) {
    ...
    let consolidation_lease = kxen_gui::knowledge::consolidate::acquire_session_lease(id).await?;

    if distill {
        // active run 已退出且 tombstone 已建立，此处重读 meta+messages，不能沿用删除入口的旧 meta。
        let mut meta = kxen_gui::core::session::load_meta(&sessions_dir, id).map_err(|error| format!("session not found: {error}"))?;
        let messages = kxen_gui::core::session::load_messages_checked(&sessions_dir, id)
        ...
        let message_cursor =
            kxen_gui::core::session::message_cursor(&messages).map_err(|error| format!("session message cursor unavailable: {error}"))?;
            ...
            .map(|message| {
                message
                    ...
                    .filter_map(|part| match part {
                        kxen_gui::core::session::Part::Text { text } | kxen_gui::core::session::Part::Context { text } => {
                            Some(text.as_str())
                        }
                        _ => None,
                    })
        ...
        let goal_id = kxen_gui::core::goal::Goal::focus_for_checked(&kxen_gui::core::paths::goals_dir(), Some(id))
            .map_err(|error| format!("goal state unavailable: {error}"))?
            .map(|goal| goal.id);
        let timeout =
            super::llm_compaction::provider_timeout_for_goal(goal_id.as_deref(), Some(kxen_gui::knowledge::distill::DISTILL_TIMEOUT))?
                .unwrap_or(kxen_gui::knowledge::distill::DISTILL_TIMEOUT);
        let outcome = kxen_gui::knowledge::consolidate::run_for_delete(kxen_gui::knowledge::consolidate::DeleteDistillRequest {
    ...
    drop(lifecycle);
    let transaction = match kxen_gui::core::session_recovery::lock_deletion_transaction(&sessions_dir, id) {
        Ok(transaction) => transaction,
        Err(error) => {
            return Err(team_abort_error(state, id, format!("session deletion transaction failed: {error}")));
        }
    };
    let bundle = match kxen_gui::core::session_recovery::stage(&sessions_dir, state.team.root(), &manifest, &transaction) {
        Ok(bundle) => bundle,
        Err(error) => {
    ...
    if let Err(error) = kxen_gui::core::session_recovery::purge_storage(&sessions_dir, state.team.root(), id, &transaction) {
        drop(transaction);
        return rollback_delete(state, &bundle, deletion, format!("session storage purge failed: {error}"));
    }
    drop(transaction);
    if let Err(error) = kxen_gui::core::session_recovery::discard_bundle(&bundle) {
...
pub(super) fn cleanup_references(state: &Arc<AppState>, id: &str) -> Result<(), String> {
    let lease = kxen_gui::knowledge::consolidate::try_acquire_session_lease(id)
...
fn settle_provider_attempts_for_manifest(state: &Arc<AppState>, id: &str) -> Result<(), String> {
    let mut usage = kxen_gui::core::shared::lock(&state.session_tokens);
    for warning in kxen_gui::core::usage::reconcile_provider_attempts_for_session(&mut usage, id)
...
fn prepare_recovery_manifest(
    state: &Arc<AppState>,
    id: &str,
    lease: &kxen_gui::knowledge::consolidate::SessionLease,
) -> Result<kxen_gui::core::session_recovery::RecoveryManifest, String> {
    for diagnostic in kxen_gui::knowledge::consolidate::settle_for_discard_leased(lease, id, &state.session_tokens)
...
fn cleanup_references_leased(
    ...
    kxen_gui::knowledge::consolidate::discard_for_session_leased(lease, id, &state.session_tokens)
    ...
    kxen_gui::core::schedule::remove_by_session(id)?;
    kxen_gui::core::goal::Goal::remove_for_session_checked(&kxen_gui::core::paths::goals_dir(), id)
        .map_err(|error| format!("remove session goals: {error}"))?;
    state.team.drop_session(id)?;
    state.agents.drop_session(id);
    kxen_gui::voice::drop_session(id);
    state.drop_extras(id);
    state.picked_files.drop_session(id);
    kxen_gui::tools::snapshot::drop_session(&state.session_snapshots, id);
    kxen_gui::core::shared::lock(&state.session_involved).remove(id);
    kxen_gui::core::shared::lock(&state.session_last_input).remove(id);
    {
        let mut usage = kxen_gui::core::shared::lock(&state.session_tokens);
        let previous = usage.remove(id);
        if let Err(error) = kxen_gui::core::usage::persist_committed(&usage) {
            if !error.committed() {
            ...
            kxen_gui::core::usage::persist_committed(&usage)
    ...
    drop(foreground);
    let mut notifications = kxen_gui::core::shared::lock(&state.notifications);
    let previous_notifications = notifications.clone();
    notifications.retain(|notice| notice.session_id.as_deref() != Some(id));
    if let Err(error) = kxen_gui::core::notifications::persist_checked(&notifications) {
...
fn rollback_delete(
    ...
    deletion: kxen_gui::core::session_recovery::DeletionGuard,
    error: String,
) -> Result<Value, String> {
