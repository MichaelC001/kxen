...
pub(in crate::ws) fn record_schedule_terminal(
    ...
    let bound_session = match kxen_gui::core::schedule::job_session(job_id) {
    ...
    let _lifecycle = match kxen_gui::core::session_lifecycle::admit_mutation(&kxen_gui::core::paths::sessions_dir(), &bound_session) {
    ...
    if let Err(error) = kxen_gui::core::schedule::record(job_id, ok, error) {
        tracing::error!(%error, cron_job_id = job_id, "cron history save failed");
        state.bus.publish(kxen_gui::core::event::Event::notify(format!("定时任务执行历史保存失败：{error}"), Some(session_id.to_string())));
        return Err(error);
    }
    Ok(())
}

pub(super) fn schedule_result(terminal: &kxen_gui::agent::agent_loop::AgentEvent) -> (bool, Option<String>) {
