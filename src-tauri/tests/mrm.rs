...
fn config_with(
    ...
    providers: &[(&str, Option<u32>, Option<u32>)],
    global: u32,
) -> Config {
    Config {
        roles: roles
...
async fn resolve_and_degrade() {
    let mrm = ModelResourceManager::new(config_with(
        &[
            ("thinking", "anthropic", "claude", None),
            ("execution", "xai", "grok", None),
            ("planning", "xai", "grok", None),
        ],
...
async fn unbound_role_falls_back_to_execution() {
    ...
    let r = mrm
        .resolve("observer", &store())
        .await
        .expect("observer 应回落 execution");
...
async fn acquire_role_is_atomic_and_mutually_exclusive() {
    ...
    for _ in 0..4 {
        ...
        handles.push(tokio::spawn(async move {
            m.acquire_role("thinking", &AuthStore::new()).await
        }));
    }
    ...
    let count = |p: &str| {
        grants
            .iter()
            .filter(|g| g.as_ref().is_some_and(|g| g.resolved.provider == p))
            .count()
    };
    ...
    let degraded = grants
        .iter()
        .flatten()
        .find(|g| g.resolved.provider == "xai")
        .unwrap();
    assert_eq!(degraded.resolved.degraded_from.as_deref(), Some("thinking"));
    // 释放后立即可再占（guard RAII）
    drop(grants);
    let g = mrm
        .acquire_role("thinking", &AuthStore::new())
        .await
        .unwrap();
