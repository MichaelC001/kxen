...
use kxen_gui::agent::agent_loop::{AgentContext, dispatch_tool};
use kxen_gui::agent::background::{NotifyRouter, drain_to_session_in, kick_late, notification_text, notifications_message, set_late_kick};
use kxen_gui::core::config::{Config, Limits, ProviderLimit, RoleBinding};
use kxen_gui::llm::mrm::ModelResourceManager;
use kxen_gui::llm::{Delta, ModelRef, StreamFn};
use serde_json::json;
...
fn router_drains_before_close_and_redirects_after() {
    ...
    router
        .close(Arc::new(move |notice| {
            kxen_gui::core::shared::lock(&c).push(notice.text);
            Ok(())
        }))
        .unwrap();
    router.notify("b".into()).unwrap();
    assert_eq!(kxen_gui::core::shared::lock(&collected).as_slice(), &["b".to_string()], "close 后通知必须直投 late 闭包");
}
...
fn close_flushes_leftover_merged_into_late() {
    ...
    router
        ...
        .unwrap();
    let got = kxen_gui::core::shared::lock(&collected).clone();
...
fn notifications_message_merges_paths_into_one_user_message() {
    ...
    assert!(matches!(msg.role, kxen_gui::llm::types::Role::User));
    assert!(
...
fn drain_to_session_persists_notes_as_user_messages() {
    ...
    let session = kxen_gui::core::session::create(&dir, "/tmp").expect("create session");
    ...
    let stored = kxen_gui::core::session::load_messages(&dir, &session.id);
    assert_eq!(stored.len(), 2, "每条通知独立落盘: {stored:?}");
    for (m, prefix) in stored.iter().zip(["[task notification]", "[teammate w]"]) {
        assert!(matches!(m.role, kxen_gui::core::session::Role::User), "通知落盘必须是 user 角色");
        match m.parts.first() {
            Some(kxen_gui::core::session::Part::Text { text }) => assert!(text.starts_with(prefix), "来源前缀保留: {text}"),
    ...
    assert_eq!(kxen_gui::core::session::load_messages(&dir, &session.id).len(), 2);
...
fn active_drain_retains_stable_delivery_until_session_commit_succeeds() {
    ...
    let stored = kxen_gui::core::session::load_messages_checked(&dir, &session.id).unwrap();
...
fn close_requeues_an_already_persisted_notice_without_id_collision() {
    ...
    let session = kxen_gui::core::session::create(&dir, "/tmp").unwrap();
    let pending = Arc::new(kxen_gui::core::pending_queue::PendingQueues::new(dir.clone()));
    let router = NotifyRouter::new_for_session(dir.clone(), session.id.clone());
    ...
    router.close(Arc::new(move |notice| kxen_gui::agent::background::deliver_late(&p, &fallback_dir, &sid, notice).map(|_| ()))).unwrap();
    ...
    let mut message = kxen_gui::core::session::new_message(
        ...
        vec![kxen_gui::core::session::Part::Text { text: claimed.text.clone() }],
    );
...
fn late_kick_fires_after_enqueue() {
    ...
    router
        .close(Arc::new(move |notice| {
            match kxen_gui::agent::background::deliver_late(&p, &fallback_dir, "s1", notice)? {
                kxen_gui::agent::background::LateDelivery::Queued => kick_late("s1"),
                kxen_gui::agent::background::LateDelivery::Preserved { warning } => return Err(warning),
            }
            Ok(())
        }))
        .unwrap();
    ...
    let stored = kxen_gui::core::session::load_messages_checked(&dir, "s1").unwrap();
...
fn late_delivery_falls_back_to_idempotent_session_message_when_queue_is_blocked() {
    ...
    let session = kxen_gui::core::session::create(&dir, "/tmp").unwrap();
    std::fs::write(kxen_gui::core::pending_queue::file_path(&dir, &session.id), "not json").unwrap();
    let pending = kxen_gui::core::pending_queue::PendingQueues::new(dir.clone());
    assert!(pending.restore().is_empty());
    assert_eq!(pending.blocked().len(), 1);

    let delivery = kxen_gui::agent::background::deliver_late(
        &pending,
        &dir,
        &session.id,
        kxen_gui::agent::background::RoutedNotice::new("late result".into()),
    )
    .unwrap();

    let kxen_gui::agent::background::LateDelivery::Preserved { warning } = delivery else {
        panic!("blocked queue must use the durable session fallback");
    };
    assert!(warning.contains("已直接保存到 Session"), "{warning}");
    let messages = kxen_gui::core::session::load_messages_checked(&dir, &session.id).unwrap();
    assert_eq!(messages.len(), 1);
    assert!(messages[0].id.starts_with("queue_"));
    assert!(matches!(messages[0].parts.first(), Some(kxen_gui::core::session::Part::Text { text }) if text == "late result"));
    std::fs::remove_dir_all(dir).ok();
}
