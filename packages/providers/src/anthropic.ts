...
async function readClaudeCodeCredentials(): Promise<Credential | undefined> {
	const credFile = Bun.file(join(homedir(), '.claude', '.credentials.json'));
...
registerProviderAuth({
	...
	async resolve({ authPath, env }) {
		...
		if (imported) {
			const existing = await readCredential(authPath, 'anthropic');
			const fresh =
				!existing ||
				existing.type !== 'oauth' ||
				(imported.type === 'oauth' &&
			...
			if (fresh) {
				await writeCredential(authPath, 'anthropic', imported);
				return imported;
			}
			return existing;
		}
		...
		if (env.ANTHROPIC_API_KEY) {
			...
			await writeCredential(authPath, 'anthropic', cred);
			return cred;
		}
		return undefined;
	},
