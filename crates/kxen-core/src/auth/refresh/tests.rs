...
fn grant_params<'a>(url: &'a str, refresh: &'a str) -> grant::GrantParams<'a> {
    grant::GrantParams { url, client_id: "cid", client_secret: None, style: GrantStyle::Json, refresh, acc_id: &None }
}
...
fn grant_success_updates_store_and_file() {
    ...
    let rt = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap();
    rt.block_on(run_grant(&mut store, &key, grant_params(&url, "old-refresh"))).unwrap();
...
fn grant_http_failure_returns_false() {
    ...
    assert!(rt.block_on(run_grant(&mut store, &key, grant_params(&url, "old-refresh"))).is_err());
...
fn grant_persistence_failure_returns_false_without_publishing_refresh() {
    ...
    let rt = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap();
    assert!(rt.block_on(run_grant_to(&mut store, &key, grant_params(&url, "old-refresh"), &impossible_auth_file,)).is_err());
    assert!(matches!(store.get(&key), Some(CredentialKind::Oauth { access, .. }) if access == "old-access"));
