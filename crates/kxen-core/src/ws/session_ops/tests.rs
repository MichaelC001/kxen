...
fn checkpoint_label_maps_to_nearest_user_message() {
    use kxen_core::core::session::{Message, Part, Role};
...
fn workspace_snapshot_invalidation_covers_every_session() {
    use kxen_core::core::session::Session;
    ...
    let stores = Mutex::new(HashMap::from([
        ("ses_one".into(), kxen_core::tools::snapshot::SnapshotStore::default()),
        ("ses_two".into(), kxen_core::tools::snapshot::SnapshotStore::default()),
        ("ses_other".into(), kxen_core::tools::snapshot::SnapshotStore::default()),
    ]));
    ...
    let remaining: Vec<String> = kxen_core::core::shared::lock(&stores).keys().cloned().collect();
...
fn chat_model_or_fallback_arms() {
    let binding = kxen_core::core::config::RoleBinding {
...
fn model_override_load_is_fail_closed_for_named_sessions() {
    ...
    let valid = Session {
        ...
        pinned: false,
        sort_order: None,
        model: Some(kxen_core::llm::ModelRef::new("anthropic", "claude-sonnet-4-6")),
    };
