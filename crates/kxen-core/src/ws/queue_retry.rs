...
pub(super) fn schedule_retry(state: Arc<AppState>, session_id: String) {
    let Some((ticket, delay)) = kxen_core::core::shared::lock(&RETRIES).reserve(&session_id) else {
        return;
    };
    tracing::warn!(session = session_id, delay_ms = delay.as_millis(), "pending queue retry scheduled");
    tokio::spawn(async move {
        tokio::time::sleep(delay).await;
        if !kxen_core::core::shared::lock(&RETRIES).take_due(&session_id, ticket) {
            return;
        }
...
pub(super) fn reset_retry(session_id: &str) {
    kxen_core::core::shared::lock(&RETRIES).reset(session_id);
}
