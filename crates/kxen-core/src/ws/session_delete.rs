...
pub(super) async fn delete(params: &Value, state: &Arc<AppState>) -> Result<Value, String> {
    ...
    let (mut deletion, active) = {
        ...
        let deletion = kxen_core::core::session_recovery::begin_deletion(&sessions_dir, id)?;
    ...
    while kxen_core::core::shared::lock(&state.active_runs).contains_key(id) {
    ...
    let consolidation_lease = kxen_core::knowledge::consolidate::acquire_session_lease(id).await?;

    if distill {
        // active run 已退出且 tombstone 已建立，此处重读 meta+messages，不能沿用删除入口的旧 meta。
        let mut meta = kxen_core::core::session::load_meta(&sessions_dir, id).map_err(|error| format!("session not found: {error}"))?;
        let messages = kxen_core::core::session::load_messages_checked(&sessions_dir, id)
        ...
        let goal_id = kxen_core::core::goal::Goal::focus_for_checked(&kxen_core::core::paths::KxenPaths::user().goals_dir(), Some(id))
            ...
            .map(|goal| goal.id);
        let timeout =
            super::llm_compaction::provider_timeout_for_goal(goal_id.as_deref(), Some(kxen_core::knowledge::distill::DISTILL_TIMEOUT))?
                .unwrap_or(kxen_core::knowledge::distill::DISTILL_TIMEOUT);
        let outcome = kxen_core::knowledge::consolidate::run_for_delete(kxen_core::knowledge::consolidate::DeleteDistillRequest {
    ...
    drop(lifecycle);
    let transaction = match kxen_core::core::session_recovery::lock_deletion_transaction(&sessions_dir, id) {
        Ok(transaction) => transaction,
        Err(error) => {
            return Err(team_abort_error(state, id, format!("session deletion transaction failed: {error}")));
        }
    };
    let bundle = match kxen_core::core::session_recovery::stage(&sessions_dir, state.team.root(), &manifest, &transaction) {
        Ok(bundle) => bundle,
        Err(error) => {
    ...
    if let Err(error) = kxen_core::core::session_recovery::purge_storage(&sessions_dir, state.team.root(), id, &transaction) {
        drop(transaction);
        return rollback_delete(state, &bundle, deletion, format!("session storage purge failed: {error}"));
    }
    drop(transaction);
    if let Err(error) = kxen_core::core::session_recovery::discard_bundle(&bundle) {
...
pub(super) fn cleanup_references(state: &Arc<AppState>, id: &str) -> Result<(), String> {
    let lease = kxen_core::knowledge::consolidate::try_acquire_session_lease(id)
...
fn settle_provider_attempts_for_manifest(state: &Arc<AppState>, id: &str) -> Result<(), String> {
    ...
    for warning in kxen_core::core::usage::reconcile_provider_attempts_for_session(&mut usage, id)
...
fn prepare_recovery_manifest(
    state: &Arc<AppState>,
    id: &str,
    lease: &kxen_core::knowledge::consolidate::SessionLease,
) -> Result<kxen_core::core::session_recovery::RecoveryManifest, String> {
    for diagnostic in kxen_core::knowledge::consolidate::settle_for_discard_leased(lease, id, &state.session_tokens)
...
fn cleanup_references_leased(
    ...
    kxen_core::knowledge::consolidate::discard_for_session_leased(lease, id, &state.session_tokens)
    ...
    kxen_core::voice::drop_session(id);
    ...
    drop(foreground);
    let mut notifications = kxen_core::core::shared::lock(&state.notifications);
    let previous_notifications = notifications.clone();
    notifications.retain(|notice| notice.session_id.as_deref() != Some(id));
    if let Err(error) = kxen_core::core::notifications::persist_checked(&notifications) {
...
fn rollback_delete(
    ...
    deletion: kxen_core::core::session_recovery::DeletionGuard,
    error: String,
) -> Result<Value, String> {
