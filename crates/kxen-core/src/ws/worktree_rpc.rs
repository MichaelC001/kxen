...
pub(super) async fn try_handle(method: &str, params: &Value, state: &Arc<AppState>) -> Result<Value, String> {
    match method {
        "worktree.list" => {
            ...
            let infos = kxen_core::tools::worktree::list(&dir).await?;
            Ok(json!(
                infos
                    .iter()
                    .map(|info| json!({
        ...
        "worktree.create" => {
            ...
            let dir = kxen_core::core::shared::read(&state.active_workspace).clone();
            let info = kxen_core::tools::worktree::create(&dir, name).await?;
            Ok(json!({
                "name": info.name,
                "path": info.path.to_string_lossy(),
                "branch": info.branch
        ...
        "worktree.status" => {
            ...
            let resolved = kxen_core::tools::worktree::resolve_in_workspace(path, &dir, &grants)?;
            Ok(json!(kxen_core::tools::worktree::status(&resolved).await?))
        }
        "diff.status" => {
            let dir = workspace_for_params(params, state)?;
            Ok(json!(kxen_core::tools::worktree::status(&dir).await?))
        }
        "diff.agent_status" => {
            let id = params.get("session_id").and_then(Value::as_str).ok_or("missing session_id")?;
            let snapshot = kxen_core::core::shared::lock(&state.session_snapshots).get(id).cloned();
        ...
        "diff.agent_file" => {
            let id = params.get("session_id").and_then(Value::as_str).ok_or("missing session_id")?;
            let path = params.get("path").and_then(Value::as_str).ok_or("missing path")?;
            let store = kxen_core::core::shared::lock(&state.session_snapshots).get(id).cloned();
        ...
        "diff.file" => {
            ...
            let grants = session_grants(params, state);
            let resolved = kxen_core::tools::worktree::resolve_in_workspace(path, &dir, &grants)?;
            Ok(json!(kxen_core::tools::worktree::diff_file(&dir, &resolved.to_string_lossy()).await?))
        }
