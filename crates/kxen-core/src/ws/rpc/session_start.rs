...
pub(super) fn spawn(
    hooks: std::sync::Arc<kxen_core::tools::hooks::HookRunner>,
    approvals: std::sync::Arc<kxen_core::agent::approval::ApprovalBroker>,
...
mod tests {
    ...
    async fn spawn_returns_before_approval_and_pending_is_session_recoverable() {
        let config: kxen_core::core::config::Config = toml::from_str(
            r#"
[[hooks.session_start]]
...
"#,
        ...
        .unwrap();
        let hooks = std::sync::Arc::new(kxen_core::tools::hooks::HookRunner::from_config(&config, std::path::Path::new("/tmp")));
        let broker = std::sync::Arc::new(kxen_core::agent::approval::ApprovalBroker::with_timeout(std::time::Duration::from_secs(5)));
        let bus = kxen_core::core::event::EventBus::new(8);
