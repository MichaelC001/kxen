...
pub(super) async fn handle(method: &str, params: &Value, state: &Arc<AppState>) -> Result<Value, String> {
    match method {
        "provider.list" => {
            let out: Vec<Value> = kxen_core::providers::all()
                .iter()
                .map(|spec| {
                    json!({
                        "key": spec.key,
                        ...
                        "oauth_login": kxen_core::auth::oauth_login::spec_for(spec.key).is_some(),
                    })
                })
                .collect();
            Ok(json!(out))
        ...
        "provider.oauth_begin" => {
            ...
            let info = kxen_core::auth::oauth_login::begin_login(provider, account, on_success).await?;
        ...
        "provider.oauth_wait" => {
            ...
            kxen_core::auth::oauth_login::await_login(session, manual_code)
        }
        "provider.oauth_cancel" => {
            let session = params.get("session").and_then(Value::as_str).ok_or("missing session")?;
            Ok(kxen_core::auth::oauth_login::cancel_login(session))
        }
        "provider.probe_models" => {
