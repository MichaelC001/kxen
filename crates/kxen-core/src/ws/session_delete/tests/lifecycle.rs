...
fn goal_for(session_id: &str, id: &str, objective: &str) -> kxen_core::core::goal::Goal {
...
async fn lifecycle_barrier_orders_mutations_manifest_and_cleanup() {
    ...
    let first = kxen_core::core::session::create(&sessions, workspace.to_str().unwrap()).unwrap();
    let mutation = kxen_core::core::session_lifecycle::admit_mutation(&sessions, &first.id).unwrap();
    let first_goal = goal_for(&first.id, "goal_lifecycle_first", "mutation committed first");
    first_goal.save(&goals).unwrap();
    kxen_core::core::schedule::add("*/5 * * * *", "first", &first.id, false).unwrap();
    ...
    assert!(!kxen_core::core::session_recovery::is_tombstoned(&sessions, &first.id).unwrap());
    drop(mutation);
    assert_eq!(tokio::time::timeout(std::time::Duration::from_secs(5), &mut deleting).await.unwrap().unwrap(), Value::Null);
    assert!(kxen_core::core::goal::Goal::load(&goals, &first_goal.id).is_err());
    assert!(kxen_core::core::schedule::list().unwrap().iter().all(|job| job.session_id != first.id));
    assert!(kxen_core::core::session_lifecycle::admit_mutation(&sessions, &first.id).is_err());

    // tombstone 建立后，RPC Goal mutation 和 schedule admission 都必须 fail closed。
    let second = kxen_core::core::session::create(&sessions, workspace.to_str().unwrap()).unwrap();
    let second_goal = goal_for(&second.id, "goal_lifecycle_tombstone", "tombstone rejects mutation");
    second_goal.save(&goals).unwrap();
    let second_job = kxen_core::core::schedule::add("*/5 * * * *", "second", &second.id, false).unwrap();
    let lease = kxen_core::knowledge::consolidate::acquire_session_lease(&second.id).await.unwrap();
    let params = json!({ "id": second.id, "distill": false });
    let deleting = delete(&params, &state);
    tokio::pin!(deleting);
    assert!(tokio::time::timeout(std::time::Duration::from_millis(50), &mut deleting).await.is_err());
    assert!(kxen_core::core::session_recovery::is_tombstoned(&sessions, &second.id).unwrap());
    ...
    assert!(kxen_core::core::session_lifecycle::admit_mutation(&sessions, &second.id).is_err());
    assert!(kxen_core::core::session_lifecycle::admit_schedule_mutation(&second_job.id).is_err());
    drop(lease);
    assert_eq!(tokio::time::timeout(std::time::Duration::from_secs(5), &mut deleting).await.unwrap().unwrap(), Value::Null);
    assert!(kxen_core::core::goal::Goal::load(&goals, &second_goal.id).is_err());
    assert!(kxen_core::core::schedule::list().unwrap().iter().all(|job| job.session_id != second.id));

    // manifest 必须包含 barrier 前已提交的新值；快照后的旧恢复包不能覆盖更晚更新，因为 tombstone 拒绝它。
    let third = kxen_core::core::session::create(&sessions, workspace.to_str().unwrap()).unwrap();
    let mut third_goal = goal_for(&third.id, "goal_lifecycle_manifest", "before mutation");
    third_goal.save(&goals).unwrap();
    let third_job = kxen_core::core::schedule::add("*/5 * * * *", "third", &third.id, false).unwrap();
    let mutation = kxen_core::core::session_lifecycle::admit_mutation(&sessions, &third.id).unwrap();
    third_goal.contract.objective = "committed before manifest".into();
    third_goal.save(&goals).unwrap();
    assert!(kxen_core::core::schedule::set_enabled(&third_job.id, false).unwrap());
    let lifecycle_delete = kxen_core::core::session_lifecycle::begin_deletion(&third.id);
    ...
    let tombstone = kxen_core::core::session_recovery::begin_deletion(&sessions, &third.id).unwrap();
    ...
    assert!(error.contains("deletion in progress"));
    assert!(kxen_core::core::session_lifecycle::admit_schedule_mutation(&third_job.id).is_err());
