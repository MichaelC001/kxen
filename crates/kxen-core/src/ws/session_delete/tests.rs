...
fn recovery_manifest_captures_blocked_knowledge_usage_before_cleanup() {
    ...
    .unwrap();
    let lease = kxen_core::knowledge::consolidate::try_acquire_session_lease(&session.id).unwrap();
...
async fn delete_transaction_commits_storage_and_reference_cleanup() {
    ...
    let message = kxen_core::core::session::new_message(
        ...
        vec![kxen_core::core::session::Part::Text { text: "delete me".into() }],
    );
    kxen_core::core::session::append_message(&sessions, &message).unwrap();
    state.pending_messages.enqueue(&session.id, "queued".into(), vec![], vec![]).unwrap();
    kxen_core::core::shared::lock(&state.session_tokens)
        .insert(session.id.clone(), kxen_core::core::usage::SessionUsage { input: 4, output: 2, ..Default::default() });
    ...
    kxen_core::core::shared::lock(&state.session_last_input).insert(session.id.clone(), 4);
    *state.foreground_session.write().unwrap() = session.id.clone();
    ...
    assert!(!kxen_core::core::shared::lock(&state.session_last_input).contains_key(&session.id));
...
async fn delete_stops_before_goal_and_usage_removal_when_pending_metering_cannot_settle() {
    ...
    std::fs::create_dir_all(&workspace).unwrap();
    let session = kxen_core::core::session::create(&sessions, workspace.to_str().unwrap()).unwrap();
    kxen_core::core::shared::lock(&state.session_tokens).insert(session.id.clone(), Default::default());
    let mut goal = kxen_core::core::goal::Goal::create(
        kxen_core::core::goal::GoalContract {
    ...
    assert!(!kxen_core::core::session_recovery::is_tombstoned(&sessions, &session.id).unwrap());
    assert!(kxen_core::core::session::load_meta(&sessions, &session.id).is_ok(), "failed delete must keep the Session usable");
...
async fn delete_establishes_tombstone_then_waits_for_active_consolidation_lease() {
    ...
    std::fs::create_dir_all(&workspace).unwrap();
    let session = kxen_core::core::session::create(&sessions, workspace.to_str().unwrap()).unwrap();
    let lease = kxen_core::knowledge::consolidate::acquire_session_lease(&session.id).await.unwrap();
