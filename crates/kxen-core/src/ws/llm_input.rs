...
pub(super) struct PrepareUserInput<'a> {
    ...
    pub(super) context: Vec<kxen_core::agent::context::ContextItem>,
    pub(super) images: Vec<kxen_core::llm::types::ImagePart>,
...
pub(super) fn expand_command(text: String, session_path: &Path) -> String {
    ...
    kxen_core::agent::commands::expand(session_path, name, args).unwrap_or(text)
}

pub(super) async fn assemble_context(
    context: Vec<kxen_core::agent::context::ContextItem>,
    images: &mut Vec<kxen_core::llm::types::ImagePart>,
    session_path: &Path,
    ...
    for item in context {
        let is_image = match &item {
            kxen_core::agent::context::ContextItem::Web { url } | kxen_core::agent::context::ContextItem::Docs { url } => {
                if let Some(image) = kxen_core::agent::context::fetch_image_url(url).await {
                    images.push(image);
                    true
                } else {
                    false
    ...
    if text_items.is_empty() {
        ...
        kxen_core::agent::context::build_context(&text_items, session_path, Some(picked)).await
    }
}

fn persisted_context_items(items: &[kxen_core::agent::context::ContextItem]) -> Vec<kxen_core::agent::context::ContextItem> {
    items
        .iter()
        .map(|item| match item {
            kxen_core::agent::context::ContextItem::Web { url } => {
                kxen_core::agent::context::ContextItem::Web { url: kxen_core::core::net_security::safe_endpoint_label(url) }
            }
            kxen_core::agent::context::ContextItem::Docs { url } => {
                kxen_core::agent::context::ContextItem::Docs { url: kxen_core::core::net_security::safe_endpoint_label(url) }
            }
            other => other.clone(),
        })
        .collect()
...
pub(super) async fn prepare_user(input: PrepareUserInput<'_>) -> Result<PreparedUser, String> {
    ...
    if let Some((delivery_id, _)) = delivery
        && let Some(message) = kxen_core::core::session::load_messages_checked(sessions_dir, session_id)
    ...
    let mut message = kxen_core::core::session::new_message(session_id, kxen_core::core::session::Role::User, parts);
...
fn replay_persisted_user(message: kxen_core::core::session::Message) -> Result<PreparedUser, String> {
    ...
    for part in &message.parts {
        match part {
            ...
            kxen_core::core::session::Part::Image { media_type, data } => {
                images.push(kxen_core::llm::types::ImagePart { media_type: media_type.clone(), data: data.clone() });
            }
...
mod tests {
    ...
    fn persisted_context_keeps_typed_sources_without_url_secrets() {
        let items = persisted_context_items(&[
            kxen_core::agent::context::ContextItem::File { path: "src/main.rs".into() },
            kxen_core::agent::context::ContextItem::Web { url: "https://user:pass@example.com/docs?q=1&api_key=SECRET#fragment".into() },
        ]);
    ...
    async fn context_assembly_handles_empty_and_local_text_items() {
        ...
        let (text, failures) = assemble_context(
            vec![kxen_core::agent::context::ContextItem::Note { text: "known context".into() }],
            &mut images,
            &root,
            &picked,
        )
    ...
    async fn queued_replay_uses_the_committed_message_snapshot() {
        ...
        let mut stored = kxen_core::core::session::new_message(
            &session.id,
            kxen_core::core::session::Role::User,
            vec![
                kxen_core::core::session::Part::Text { text: "original".into() },
                kxen_core::core::session::Part::Context { text: "snapshot".into() },
                kxen_core::core::session::Part::Image { media_type: "image/png".into(), data: "AA==".into() },
            ],
        );
        stored.id = "queue_stable".into();
        stored.created_at = 7;
        kxen_core::core::session::append_message(&root, &stored).unwrap();
        ...
        let prepared = prepare_user(PrepareUserInput {
            ...
            picked: &HashSet::new(),
            text: "changed".into(),
            context: vec![kxen_core::agent::context::ContextItem::Note { text: "changed context".into() }],
            images: Vec::new(),
