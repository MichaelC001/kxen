...
async fn run_llm_inner(input: spawn::RunInput, preclaimed: Option<kxen_core::agent::cancel::CancelToken>) {
    ...
    let session_meta = match ses::load_meta(&sessions_dir, &session_id) {
        ...
        Err(e) => {
            ...
            early.finish(
                ...
                false,
                None,
                kxen_core::agent::agent_loop::AgentEvent::Error { message: format!("session unavailable: {e}") },
            );
            return;
        }
    };
    ...
    let _run_guard = kxen_core::core::rewind_lock::run_guard(&session_dir, &session_id, &state.bus).await;
    let runtime = match state.workspace_runtimes.ready(&session_path).await {
        ...
        Err(e) => {
            ...
            let delivery = queue_delivery_id
                .as_deref()
                .map_or(DeliveryOutcome::Direct, |delivery_id| super::queue_delivery::release(&state, &session_id, delivery_id));
            early.finish(delivery, false, None, kxen_core::agent::agent_loop::AgentEvent::Error { message: e });
            return;
        }
    };
    ...
    let delivery_input = match (queue_delivery_id.as_deref(), queue_created_at) {
        ...
        _ => {
            early.finish_blocked(
                ...
                kxen_core::agent::agent_loop::AgentEvent::Error { message: "queued delivery is missing its durable creation time".into() },
            );
            return;
        }
    };
    ...
    .await
    {
        ...
        Err(message) => {
            early.finish_blocked(
                DeliveryOutcome::pending(queue_delivery_id.as_deref()),
                kxen_core::agent::agent_loop::AgentEvent::Error { message },
            );
            return;
        }
    };
    for f in &prepared.failures {
        state.bus.publish(kxen_core::core::event::Event::notify(format!("引用读取失败：{f}"), Some(session_id.clone())));
    }
    ...
    let stored_history = match ses::load_history_checked(&sessions_dir, &session_id) {
        ...
        Err(error) => {
            early.finish(
                ...
                true,
                None,
                kxen_core::agent::agent_loop::AgentEvent::Error { message: format!("session history unavailable: {error}") },
            );
            return;
        }
    };
    ...
    let inbox = match state.team.drain_lead_inbox(&session_id) {
        ...
        Err(error) => {
            early.finish(
                ...
                true,
                None,
                kxen_core::agent::agent_loop::AgentEvent::Error { message: format!("team state unavailable: {error}") },
            );
            return;
        }
    };
    ...
    if with_images {
        match messages.iter().rposition(|m| m.role == kxen_core::llm::types::Role::User && m.content == text) {
    ...
    let mut ctx = kxen_core::agent::agent_loop::AgentContext {
        registry,
        tracker: {
            let mut t = kxen_core::tools::fs_tool::FileTracker::default();
            // 会话级改动快照：改动面板「本会话 agent 改了什么」的数据源
            t.snapshots = kxen_core::core::shared::lock(&state.session_snapshots).entry(session_id.clone()).or_default().clone();
            t
        },
        workdir,
    ...
    let outcome = kxen_core::agent::agent_loop::run_turn(&mut ctx, &mut messages).await;
