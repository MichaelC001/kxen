...
type ActiveRuns = Mutex<std::collections::HashMap<String, kxen_core::agent::cancel::CancelToken>>;
...
pub(super) fn claim_run(
    ...
    session_id: &str,
) -> Result<Option<kxen_core::agent::cancel::CancelToken>, String> {
...
pub(super) fn claim_run_with(
    ...
    let mut runs = kxen_core::core::shared::lock(active_runs);
    if kxen_core::core::session_recovery::is_tombstoned(sessions_dir, session_id)? {
...
) -> Result<Option<(T, kxen_core::agent::cancel::CancelToken)>, String> {
    let mut runs = kxen_core::core::shared::lock(active_runs);
    if kxen_core::core::session_recovery::is_tombstoned(sessions_dir, session_id)? {
        return Err(format!("session deletion in progress: {session_id}"));
    }
...
pub(super) fn claim_queued_handoff<T>(
    ...
    sessions_dir: &std::path::Path,
    session_id: &str,
    current: &kxen_core::agent::cancel::CancelToken,
    claim: impl FnOnce() -> Result<Option<T>, String>,
) -> Result<Option<(T, kxen_core::agent::cancel::CancelToken)>, String> {
    let mut runs = kxen_core::core::shared::lock(active_runs);
    if kxen_core::core::session_recovery::is_tombstoned(sessions_dir, session_id)? {
    ...
    let Some(delivery) = claim()? else { return Ok(None) };
    let next = kxen_core::agent::cancel::CancelToken::new();
    runs.insert(session_id.to_string(), next.clone());
    Ok(Some((delivery, next)))
}

pub(super) fn is_current(active_runs: &ActiveRuns, session_id: &str, token: &kxen_core::agent::cancel::CancelToken) -> bool {
    kxen_core::core::shared::lock(active_runs).get(session_id).is_some_and(|active| active.same_generation(token))
}
...
pub(super) struct RunSlot {
    ...
    pub cancel: kxen_core::agent::cancel::CancelToken,
}

impl Drop for RunSlot {
    fn drop(&mut self) {
        kxen_core::agent::cancel::remove_if_current(
            &mut kxen_core::core::shared::lock(&self.state.active_runs),
            &self.session_id,
            &self.cancel,
        );
    }
...
mod tests {
    ...
    fn concurrent_claims_yield_single_winner() {
        ...
        assert_eq!(kxen_core::core::shared::lock(&*runs).len(), 1);
    }
    ...
    fn released_slot_can_be_claimed_again() {
        ...
        let intruder = kxen_core::agent::cancel::CancelToken::new();
        kxen_core::agent::cancel::remove_if_current(&mut kxen_core::core::shared::lock(&runs), "s", &intruder);
        assert!(claim_run(&runs, &sessions, "s").unwrap().is_none());
        // 本 run 收尾摘除后槽位可再抢
        kxen_core::agent::cancel::remove_if_current(&mut kxen_core::core::shared::lock(&runs), "s", &first);
    ...
    fn deletion_tombstone_blocks_claim() {
        ...
        let guard = kxen_core::core::session_recovery::begin_deletion(&sessions, "ses_one").unwrap();
        ...
        assert!(kxen_core::core::shared::lock(&runs).is_empty());
        let abort_error =
    ...
    fn queue_handoff_replaces_generation_without_releasing_the_slot() {
        ...
        kxen_core::agent::cancel::remove_if_current(&mut kxen_core::core::shared::lock(&runs), "s", &current);
        ...
        for _ in 0..8 {
            ...
            handles.push(std::thread::spawn(move || {
                ...
                let result = claim_queued_run(&runs, &sessions, "s", || {
                    ...
                    Ok(kxen_core::core::shared::lock(&delivery).take())
                })
                .unwrap();
                if result.is_some() {
                    winners.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
