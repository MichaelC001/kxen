...
pub(super) async fn statusline_report(session_id: &str, state: &Arc<AppState>) -> Result<Value, String> {
    ...
    // git 分支（5s 缓存）
    let git_branch = {
        let cached = kxen_core::core::shared::lock(&state.git_cache).get(&workdir).cloned();
            ...
            && at.elapsed() <= std::time::Duration::from_secs(5)
        {
            ...
            let branch = std::process::Command::new("git")
                ...
                .unwrap_or_default();
            kxen_core::core::shared::lock(&state.git_cache).insert(workdir.clone(), (std::time::Instant::now(), branch.clone()));
            branch
        }
    };
    ...
    let tasks_running = if session_id.is_empty() {
        ...
        let owner = kxen_core::tools::task::TaskOwner::new(session_id, &workdir)
            .map_err(|error| format!("statusline task owner unavailable for session {session_id}: {error}"))?;
        state.registry.list(&owner).iter().filter(|task| matches!(task.status, kxen_core::tools::task::TaskStatus::Running)).count()
    };
...
fn statusline_focus(goals_dir: &std::path::Path, session_id: Option<&str>) -> Result<Option<kxen_core::core::goal::Goal>, String> {
    kxen_core::core::goal::Goal::focus_for_checked(goals_dir, session_id)
...
fn validate_role_update(role: &str, provider: &str, model: &str, fallback: Option<&str>, account: Option<&str>) -> Result<(), String> {
    kxen_core::auth::credential::validate_identity(role, "role")?;
    kxen_core::auth::credential::validate_identity(provider, "provider")?;
    kxen_core::auth::credential::validate_identity(model, "model")?;
    if let Some(fallback) = fallback.filter(|fallback| !fallback.is_empty()) {
        kxen_core::auth::credential::validate_identity(fallback, "fallback role")?;
    }
...
pub(super) fn coding_rules_report() -> Value {
    json!({
        "enabled": kxen_core::core::config::coding_rules_enabled(),
        "content": kxen_core::agent::prompt::CODING_RULES,
    })
}
