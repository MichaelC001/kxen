...
pub(super) struct CommitFailure {
    ...
    pub(super) terminal: kxen_core::agent::agent_loop::AgentEvent,
}

pub(super) fn commit_user(
    state: &Arc<AppState>,
    sessions_dir: &Path,
    message: &kxen_core::core::session::Message,
    delivery_id: Option<&str>,
) -> Result<DeliveryOutcome, CommitFailure> {
    let appended = if delivery_id.is_some() {
        kxen_core::core::session::append_message_idempotent_durable(sessions_dir, message)
    } else {
        kxen_core::core::session::append_message_durable(sessions_dir, message)
    };
    match appended {
        Ok(_) => {}
        Err(error) if error.committed() => {
            ...
            return Err(CommitFailure {
                ...
                blocked: true,
                terminal: kxen_core::agent::agent_loop::AgentEvent::Error {
        ...
        Err(error) => {
            ...
            return Err(CommitFailure {
                ...
                persist_terminal: false,
                blocked: false,
                terminal: kxen_core::agent::agent_loop::AgentEvent::Error { message: format!("session append failed: {error}") },
            });
        }
    }
    super::super::queue_delivery::settle(state, &message.session_id, delivery_id, true).map_err(|error| CommitFailure {
        ...
        blocked: false,
        terminal: kxen_core::agent::agent_loop::AgentEvent::Error { message: error.message },
    })
}
