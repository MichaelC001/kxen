...
pub(super) fn recover_restored(state: &Arc<AppState>) -> Result<Vec<String>, String> {
    ...
    for id in kxen_core::core::session_recovery::discover_tombstones(&sessions_dir)? {
        ...
        let bundle = kxen_core::core::session_recovery::bundle_path(&sessions_dir, &id);
        if let Err(error) = kxen_core::core::session_recovery::recover_discard_backup(&bundle) {
        ...
        match result.and_then(|()| kxen_core::core::session_recovery::clear_tombstone(&sessions_dir, &id)) {
            Ok(()) => {}
            Err(error) => tracing::warn!(session = id, %error, "session deletion recovery failed"),
        }
    }
    for bundle in kxen_core::core::session_recovery::discover(&sessions_dir)? {
...
fn restore_runtime_and_complete(
    ...
    if let Some(last_input) = manifest.last_input {
        kxen_core::core::shared::lock(&state.session_last_input).insert(id.clone(), last_input);
    }
...
pub(super) fn stage_manifest(state: &AppState, session_id: &str) -> Result<kxen_core::core::session_recovery::RecoveryManifest, String> {
    ...
    manifest.goals = kxen_core::core::goal::Goal::list_checked(&kxen_core::core::paths::goals_dir())
        ...
        .collect();
    manifest.usage = kxen_core::core::shared::lock(&state.session_tokens).get(session_id).cloned();
    manifest.last_input = kxen_core::core::shared::lock(&state.session_last_input).get(session_id).copied();
    Ok(manifest)
}
