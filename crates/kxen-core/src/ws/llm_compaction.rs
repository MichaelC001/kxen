...
pub(super) struct CompactionInput<'a> {
    ...
    pub(super) store: &'a kxen_core::auth::credential::AuthStore,
    pub(super) mrm: &'a kxen_core::llm::mrm::ModelResourceManager,
...
pub(super) async fn compact_if_needed(input: CompactionInput<'_>) -> Result<(), (AgentEvent, Option<ModelRef>)> {
    ...
    let history = kxen_core::core::session::load_history_checked(sessions_dir, session_id)
        .map_err(|error| (AgentEvent::Error { message: format!("session history unavailable: {error}") }, None))?;
    let messages = kxen_core::agent::compact::flatten_stored(&history);
    if !kxen_core::agent::compact::needs_compact(&messages, model) {
        return Ok(());
    }

    let timeout = provider_timeout_for_goal(goal_id, Some(kxen_core::agent::compact::COMPACT_TIMEOUT))
        .map_err(|message| (AgentEvent::Error { message }, None))?
        .expect("compaction cap always produces a timeout");

    state.bus.publish(kxen_core::core::event::Event::notify("上下文超阈值，正在自动压缩历史", Some(session_id.to_string())));
    let mut metering = CompactionMeter::begin(state, session_id, goal_id).map_err(|event| (event, None))?;
    let options = kxen_core::agent::compact::CompactSessionOptions {
        mrm: Some(mrm),
        keep_recent: 6,
        timeout,
        cancel: Some(cancel),
    ...
    match kxen_core::agent::compact::compact_session(sessions_dir, session_id, model, store, options).await {
        Ok(Some(report)) => {
        ...
        Err(kxen_core::agent::compact::CompactError::Cancelled { request_started, usage, metering_warning, model_used, .. }) => {
        ...
        Err(kxen_core::agent::compact::CompactError::Persist { message, request_started, usage, metering_warning, model_used, .. }) => {
            metering.settle(request_started, usage, metering_warning).map_err(|event| (event, model_used.clone()))?;
...
pub(super) fn provider_timeout_for_goal(
    ...
    match budget {
        kxen_core::core::goal::RuntimeBudget::Unbounded => Ok(cap),
        kxen_core::core::goal::RuntimeBudget::WallRemaining(remaining) => Ok(Some(cap.map_or(remaining, |limit| limit.min(remaining)))),
        kxen_core::core::goal::RuntimeBudget::Stop(status) => Err(format!("goal 当前状态 {} 禁止 Provider 调用", status.as_str())),
    }
}

pub(crate) struct CompactionMeter {
    reporter: kxen_core::agent::agent_loop::UsageReporter,
    attempt: kxen_core::core::usage::ProviderAttempt,
...
impl CompactionMeter {
    pub(crate) fn begin(state: &crate::AppState, session_id: &str, goal_id: Option<&str>) -> Result<Self, AgentEvent> {
        let reporter =
            kxen_core::agent::agent_loop::UsageReporter::new(session_id.to_string(), state.session_tokens.clone(), state.bus.clone());
        let attempt =
    ...
    pub(crate) fn settle(
        ...
        usage: Option<kxen_core::llm::managed::TokenUsage>,
        ...
        if !request_started {
            ...
            if let Some(warning) = warning {
                self.bus.publish(kxen_core::core::event::Event::notify(format!("用量持久化已修复：{warning}"), Some(self.session_id)));
            }
            return Ok(());
        }
        self.reporter
        ...
        for warning in outcome.durability_warnings {
            self.bus.publish(kxen_core::core::event::Event::notify(format!("用量持久化已修复：{warning}"), Some(self.session_id.clone())));
        }
        if let Some(message) = outcome.stop_message {
            return Err(AgentEvent::Error { message });
        }
        if let Some(warning) = metering_warning {
            self.bus.publish(kxen_core::core::event::Event::notify(format!("用量计量降级：{warning}"), Some(self.session_id)));
        }
        Ok(())
    }
}
...
pub(super) fn save_run_checkpoint(
    ...
    kxen_core::core::session::save_compaction(
        sessions_dir,
        session_id,
        &kxen_core::core::session::Compaction::new(boundary, summary.to_string()),
    )
...
mod tests {
    use super::*;
    use kxen_core::core::session::{self, Part, Role};
    ...
    fn run_checkpoint_keeps_uncovered_recent_messages() {
        ...
        let covered = kxen_core::agent::compact::flatten_stored(&raw[..4]);
