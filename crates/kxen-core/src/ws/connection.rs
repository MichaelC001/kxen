...
struct CallTasks {
    tasks: JoinSet<()>,
    disconnect: kxen_core::agent::cancel::CancelToken,
}
...
impl CallTasks {
        ...
        F: Future<Output = Result<Value, CallError>> + Send + 'static,
    {
        ...
        self.tasks.spawn(async move {
            kxen_core::agent::approval::scope_wait_cancellation(disconnect, response_task(id, future, outbound)).await;
        });
    }
...
mod tests {
    ...
    async fn disconnect_aborts_approval_rpc_and_broker_fails_closed() {
        let broker = Arc::new(kxen_core::agent::approval::ApprovalBroker::with_timeout(std::time::Duration::from_secs(30)));
        ...
        calls.spawn(
            ...
            async move {
                ...
                if rpc_broker.wait(&id, rx, None).await == kxen_core::agent::approval::ApprovalOutcome::Allow {
