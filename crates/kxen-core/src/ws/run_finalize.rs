...
pub(super) struct RunEnd<'a> {
    ...
    pub runtime: Arc<kxen_core::workspace_runtime::WorkspaceRuntime>,
    pub session_id: String,
    pub stream_id: String,
    pub notify: Arc<kxen_core::agent::background::NotifyRouter>,
...
pub(super) async fn finalize_run(end: RunEnd<'_>) {
    ...
    let close_result = notify.close({
        ...
        std::sync::Arc::new(move |notice: kxen_core::agent::background::RoutedNotice| {
            match kxen_core::agent::background::deliver_late(&state.pending_messages, &sessions_dir, &sid, notice)? {
                kxen_core::agent::background::LateDelivery::Queued => {
                    kxen_core::agent::background::kick_late(&sid);
                    Ok(())
                }
                kxen_core::agent::background::LateDelivery::Preserved { warning } => {
                    tracing::error!(session = sid, %warning, "late background notification used durable fallback");
                    state.bus.publish(kxen_core::core::event::Event::notify(warning, Some(sid.clone())));
                    Ok(())
                }
            }
        })
    ...
    let stop_appr = kxen_core::tools::exec::ApprovalCtx::new(
...
pub(super) fn handoff_pending(state: &Arc<AppState>, session_id: String, cancel: &kxen_core::agent::cancel::CancelToken) {
    ...
    let (q, next_cancel) = match handoff {
        ...
        Err(error) => {
            ...
            state.bus.publish(kxen_core::core::event::Event::notify(format!("队列续跑失败：{error}"), Some(session_id.clone())));
            release_current_slot(state, &session_id, cancel);
...
fn release_current_slot(state: &Arc<AppState>, session_id: &str, cancel: &kxen_core::agent::cancel::CancelToken) {
    kxen_core::agent::cancel::remove_if_current(&mut kxen_core::core::shared::lock(&state.active_runs), session_id, cancel);
}
...
pub(super) fn publish_terminal(
    ...
    bus.publish(kxen_core::core::event::Event::LlmDelta(payload));
}

pub(super) fn finish_direct(state: &Arc<AppState>, session_id: &str, stream_id: &str, terminal: kxen_core::agent::agent_loop::AgentEvent) {
