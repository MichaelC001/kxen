...
struct UsageTotals {
    ...
    unmetered_calls: u64,
    sessions: usize,
    completeness: kxen_core::core::usage::UsageCompleteness,
}

fn usage_totals(tokens: &std::collections::HashMap<String, kxen_core::core::usage::SessionUsage>) -> UsageTotals {
...
async fn handle(method: &str, params: &Value, state: &Arc<AppState>) -> Result<Value, String> {
    match method {
        ...
        "usage.overview" => {
            ...
            let daily: Vec<_> = kxen_core::core::usage_trend::recent(14)
                .into_iter()
                .map(
                    |(date, usage)| json!({ "date": date, "input": usage.input, "output": usage.output, "by_provider": usage.by_provider }),
                )
                .collect();
            let today = kxen_core::core::usage_trend::today();
            Ok(json!({
        ...
        "schedule.remove" => {
            ...
            let _lifecycle = kxen_core::core::session_lifecycle::admit_schedule_mutation(id)?;
            Ok(json!(kxen_core::core::schedule::remove(id)?))
        }
        "schedule.set_enabled" => {
            let id = params.get("id").and_then(Value::as_str).ok_or("missing id")?;
            let enabled = params.get("enabled").and_then(Value::as_bool).ok_or("missing enabled")?;
            let _lifecycle = kxen_core::core::session_lifecycle::admit_schedule_mutation(id)?;
            Ok(json!(kxen_core::core::schedule::set_enabled(id, enabled)?))
        }
        ...
        "voice.engines" => {
            ...
            Ok(json!({
                ...
                "engines": kxen_core::voice::engines(&config, &store),
            }))
        }
        "voice.set_provider_key" => {
        ...
        "voice.start" => {
            ...
            let started = kxen_core::voice::start(&voice, &store, locale, state.bus.clone(), session_id)?;
        ...
        "voice.stop" => {
            ...
            let usage_reporter = if session_id.is_empty() {
                kxen_core::agent::agent_loop::UsageReporter::new_unscoped("system_voice", state.session_tokens.clone(), state.bus.clone())
            } else {
                kxen_core::agent::agent_loop::UsageReporter::new(session_id.to_string(), state.session_tokens.clone(), state.bus.clone())
            };
            let text = kxen_core::voice::stop(&config, &store, session_id, &mrm, &usage_reporter).await?;
            Ok(json!({ "text": text }))
        }
...
async fn test_dispatch(state: &Arc<AppState>, params: &Value) -> Result<Value, String> {
    ...
    let deps = kxen_core::agent::subagent::SubagentDeps {
    ...
    let dispatch = kxen_core::agent::subagent::dispatch(
        ...
        &deps,
        kxen_core::agent::activity::AgentKind::Subagent,
    )
    .await?;
    Ok(json!({
        "role": role,
