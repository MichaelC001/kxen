...
pub(super) enum SpecialResult {
    ...
    Handled {
        ...
        persist_model: Option<kxen_core::llm::ModelRef>,
...
pub(super) async fn handle(
    ...
    sessions_dir: &Path,
    session_id: &str,
    cancel: &kxen_core::agent::cancel::CancelToken,
    goal_id: Option<&str>,
) -> SpecialResult {
...
async fn compact_session(
    ...
    cancel: &kxen_core::agent::cancel::CancelToken,
    goal_id: Option<&str>,
) -> (AgentEvent, Option<kxen_core::llm::ModelRef>) {
    ...
    let timeout = match super::llm_compaction::provider_timeout_for_goal(goal_id, Some(kxen_core::agent::compact::COMPACT_TIMEOUT)) {
        Ok(Some(timeout)) => timeout,
        Ok(None) => kxen_core::agent::compact::COMPACT_TIMEOUT,
        Err(message) => return (AgentEvent::Error { message }, None),
    };
    ...
    let (notice, model_used) = match kxen_core::agent::compact::compact_session(sessions_dir, session_id, &model, &store, options).await {
        Ok(Some(report)) => {
