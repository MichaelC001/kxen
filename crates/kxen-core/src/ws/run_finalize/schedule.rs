...
pub(in crate::ws) fn record_schedule_terminal(
    ...
    let bound_session = match kxen_core::core::schedule::job_session(job_id) {
    ...
    if let Err(error) = kxen_core::core::schedule::record(job_id, ok, error) {
        ...
        state
            .bus
            .publish(kxen_core::core::event::Event::notify(format!("定时任务执行历史保存失败：{error}"), Some(session_id.to_string())));
        return Err(error);
    }
    Ok(())
}

pub(super) fn schedule_result(terminal: &kxen_core::agent::agent_loop::AgentEvent) -> (bool, Option<String>) {
