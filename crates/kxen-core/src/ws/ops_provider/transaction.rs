...
pub(super) fn transact_custom_provider_with_runtime(
    ...
    let result = super::super::ops_config::update_toml_transaction_staged(
        config_path,
        |original_config| {
            let auth = kxen_core::auth::credential::read_auth_file(auth_path).map_err(|error| error.to_string())?;
...
fn rollback_runtime_and_stores(
    runtime: &mut kxen_core::workspace_runtime::RuntimeConfigUpdate,
    config_path: &Path,
    auth_path: &Path,
    prefix: String,
) -> String {
...
pub(crate) fn recover_custom_provider_transaction(config_path: &Path, auth_path: &Path) -> Result<(), String> {
    ...
    if matches!(journal.phase, JournalPhase::Prepared) {
        ...
        kxen_core::auth::credential::update_auth_file(auth_path, |disk| {
            for key in kxen_core::auth::credential::accounts_of(disk, &journal.provider) {
                disk.remove(&key);
            }
...
fn update_auth_with_compensation(
    ...
    let updated = kxen_core::auth::credential::update_auth_file(auth_path, |disk| {
    ...
    match updated {
        ...
        Err(error) => {
            ...
            match kxen_core::auth::credential::update_auth_file(auth_path, |disk| {
...
fn custom_auth_entries(store: &AuthStore, provider: &str) -> Vec<(String, CredentialKind)> {
    kxen_core::auth::credential::accounts_of(store, provider)
        .into_iter()
