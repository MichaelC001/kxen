use super::*;
use kxen_core::auth::ProbeOutcome::*;
use kxen_core::auth::credential::CredentialKind;
...
fn auth_commit_holds_memory_lock_until_snapshot_is_published() {
    ...
    let worker = std::thread::spawn(move || {
        commit_auth_transaction(&worker_store, || {
            ...
            Ok(kxen_core::auth::credential::AuthUpdate::Durable(persisted))
        })
        .expect("commit")
    });
...
fn set_region_validates_and_updates_api_cred() {
    let mut store = kxen_core::auth::credential::AuthStore::new();
...
fn account_crud_keeps_disk_memory_and_listing_consistent() {
    ...
    assert_eq!(kxen_core::auth::credential::read_auth_file(&auth_path).unwrap()["kimi:work"].bearer(), "secret");
...
fn account_postcommit_warning_publishes_disk_snapshot_to_memory() {
    ...
    let disk = kxen_core::auth::credential::read_auth_file(&auth_path).unwrap();
...
fn custom_provider_transaction_commits_add_and_remove_to_both_files() {
    ...
    assert_eq!(kxen_core::auth::credential::read_auth_file(&auth_path).unwrap()[provider].bearer(), "secret");
    ...
    let (persisted, warning) = transact_custom_provider(
        ...
        |store| {
            for key in kxen_core::auth::credential::accounts_of(store, provider) {
                store.remove(&key);
            }
            Ok(())
        },
    ...
    assert!(kxen_core::auth::credential::accounts_of(&persisted, provider).is_empty());
    let disk = kxen_core::auth::credential::read_auth_file(&auth_path).unwrap();
    assert!(kxen_core::auth::credential::accounts_of(&disk, provider).is_empty());
    std::fs::remove_dir_all(root).ok();
}
...
fn custom_provider_second_step_failure_restores_config_and_auth() {
    ...
    .unwrap();
    let mut original_auth = kxen_core::auth::credential::AuthStore::new();
    ...
    kxen_core::auth::credential::write_auth_file(&auth_path, &original_auth).unwrap();
    ...
    assert_eq!(kxen_core::auth::credential::read_auth_file(&auth_path).unwrap(), original_auth);
    std::fs::remove_dir_all(root).ok();
}
