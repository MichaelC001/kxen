...
fn prepared_journal_restores_both_stores() {
    ...
    kxen_core::auth::credential::write_auth_file(&auth, &AuthStore::new()).unwrap();
    ...
    assert_eq!(kxen_core::auth::credential::read_auth_file(&auth).unwrap()["custom:lab"], credential);
...
fn committed_journal_keeps_stores_and_only_clears_marker() {
    ...
    kxen_core::auth::credential::write_auth_file(&auth, &current).unwrap();
    write_journal(
        &config,
        &TransactionJournal {
    ...
    assert_eq!(kxen_core::auth::credential::read_auth_file(&auth).unwrap(), current);
    assert!(!journal_path(&config).unwrap().exists());
    std::fs::remove_dir_all(root).ok();
}
...
fn committed_marker_sync_failure_is_indeterminate_and_keeps_recovery_armed() {
    ...
    recover_custom_provider_transaction(&config, &auth).unwrap();
    assert_eq!(kxen_core::auth::credential::read_auth_file(&auth).unwrap()["custom:lab"].bearer(), "current");
    std::fs::remove_dir_all(root).ok();
}
...
fn commit_marker_failure_restores_config_auth_and_cached_runtime() {
    ...
    kxen_core::auth::credential::write_auth_file(&auth, &original_auth).unwrap();
    let runtimes = kxen_core::workspace_runtime::WorkspaceRuntimeRegistry::with_user_config(config.clone()).unwrap();
    ...
    let restored = kxen_core::core::config::Config::load(&config, None).unwrap();
    assert_eq!(restored.custom_providers["lab"].base_url, "https://old.example/v1");
    assert_eq!(kxen_core::auth::credential::read_auth_file(&auth).unwrap(), original_auth);
