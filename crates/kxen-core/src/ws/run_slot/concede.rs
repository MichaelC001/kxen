...
pub(in crate::ws) struct ConcedePayload {
    ...
    pub(in crate::ws) context: Vec<kxen_core::agent::context::ContextItem>,
    pub(in crate::ws) images: Vec<kxen_core::llm::types::ImagePart>,
}
...
pub(in crate::ws) fn concede(
    ...
    match queue_delivery_id {
        ...
        None => match state.pending_messages.enqueue(session_id, payload.text, payload.context, payload.images) {
            Ok(n) => state
                .bus
                .publish(kxen_core::core::event::Event::notify(format!("运行中，消息已排队（第 {n} 条）"), Some(session_id.to_string()))),
            Err(error) => super::super::run_finalize::finish_direct(
                state,
                session_id,
                stream_id,
                kxen_core::agent::agent_loop::AgentEvent::Error { message: format!("pending queue enqueue failed: {error}") },
            ),
        },
    }
}
