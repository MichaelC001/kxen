...
use kxen_core::core::goal::{Goal, GoalBudget, GoalContract};
        ...
        "goal.create" => {
            ...
            let contract = GoalContract {
                ...
                budget: GoalBudget {
                    ...
                    turns: kxen_core::core::goal::checked_turn_budget(params.pointer("/budget/turns").and_then(Value::as_u64))
                        .map_err(|error| error.to_string())?,
                    wall_clock_ms: params.pointer("/budget/wall_clock_ms").and_then(Value::as_u64),
                },
            };
            let id = kxen_core::core::ids::new_id("goal");
...
fn apply_transition(method: &str, goal: &mut Goal) -> Result<(), kxen_core::core::goal::GoalError> {
    match method {
...
fn publish(bus: &kxen_core::core::event::EventBus, goal: &Goal) {
...
async fn transit(
    ...
    f: impl FnOnce(&mut Goal) -> Result<(), kxen_core::core::goal::GoalError>,
) -> Result<Value, String> {
    let id = params.get("id").and_then(Value::as_str).ok_or("missing id")?;
    kxen_core::core::ids::validate_id(id)?;
    let _lifecycle = kxen_core::core::session_lifecycle::admit_goal_mutation(&dir(), id)?;
    // 与记账共用 per-id 锁：并发 adjust 与 charge 的 load-modify-save 串行化，不互相覆盖
    let lock = kxen_core::core::goal::write_lock(id);
    let _guard = kxen_core::core::shared::lock(&lock);
    let mut goal = load(id)?;
    f(&mut goal).map_err(|e| e.to_string())?;
    goal.save(&dir()).map_err(|e| e.to_string())?;
    // pause/cancel 停在飞 run：直接 cancel run 令牌即时停出，不等轮末记账发现
    if matches!(goal.status, kxen_core::core::goal::GoalStatus::Paused | kxen_core::core::goal::GoalStatus::Canceled)
        && let Some(sid) = goal.session_id.as_deref()
        && let Some(token) = kxen_core::core::shared::lock(&state.active_runs).get(sid).cloned()
    {
        token.cancel();
    }
...
mod tests {
    use super::*;
    use kxen_core::core::goal::GoalStatus;
    ...
    fn publish_emits_canonical_goal_update() {
        let bus = kxen_core::core::event::EventBus::new(4);
