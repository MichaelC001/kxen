...
fn build_core_tools() -> Vec<ToolDefinition> {
    vec![
        ...
        ToolDefinition::function(
            "tool_search",
            "Discover additional tools that are not loaded by default (progressive disclosure): delete, lsp, agent, worktree, skill, knowledge, schedule, browser, kanban_* (workspace board pipeline). Returns matching tool cards; matched tools become callable for the rest of this session.",
            json!({
                "type": "object",
                "properties": {
