...
impl DcpStore {
    ...
    pub fn load_session(&self, session_id: &str) -> Result<DcpSessionState, String> {
        ...
        if state.schema_version != DCP_SESSION_SCHEMA || state.session_id != session_id {
            return Err(format!("DCP Session identity or schema mismatch: {session_id}"));
        }
        Ok(state)
    }

    pub fn save_session(&self, state: &DcpSessionState) -> Result<(), String> {
        crate::core::ids::validate_id(&state.session_id)?;
        if state.schema_version != DCP_SESSION_SCHEMA {
            return Err(format!("unsupported DCP Session schema {}", state.schema_version));
        }
    ...
    pub fn load_run(&self, session_id: &str, run_id: &str) -> Result<DcpRunState, String> {
        ...
        if run.session_id != session_id || run.run_id != run_id {
            return Err(format!("DCPRun identity mismatch: {run_id}"));
        }
        Ok(run)
    }

    pub fn save_run(&self, run: &DcpRunState) -> Result<(), String> {
        crate::core::ids::validate_id(&run.session_id)?;
        crate::core::ids::validate_id(&run.run_id)?;
    ...
    pub fn import_bundle(&self, mut bundle: DcpSessionBundle, workspace: &Path) -> Result<DcpSessionState, String> {
        ...
        if bundle.dcp.session_id != session_id || bundle.messages.iter().any(|message| message.session_id != session_id) {
            return Err("DCP Session bundle identity mismatch".into());
        }
        ...
        for (run_id, run) in &bundle.runs {
            if run_id != &run.state.run_id || run.state.session_id != session_id {
                return Err(format!("DCPRun bundle identity mismatch: {run_id}"));
            }
