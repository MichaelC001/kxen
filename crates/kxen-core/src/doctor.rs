...
pub struct SystemHealth {
    pub mcp_ready: bool,
    pub mcp: Vec<kxen_core::mcp::ServerStatus>,
    pub lsp_root: String,
...
fn kxen_account_keys(store: &AuthStore, provider: &str) -> Vec<String> {
    kxen_core::auth::credential::accounts_of(store, provider).into_iter().filter(|k| k != provider).collect()
}
    ...
    fn report_with_system() -> DoctorReport {
        DoctorReport {
            ...
            system: Some(SystemHealth {
                mcp_ready: true,
                mcp: vec![kxen_core::mcp::ServerStatus {
                    name: "fs".into(),
                    status: "running".into(),
