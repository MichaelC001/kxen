...
pub struct AppState {
    ...
    pub bus: kxen_core::core::event::EventBus,
    pub registry: std::sync::Arc<kxen_core::tools::task::TaskRegistry>,
    /// 角色路由可热更新（设置页改角色 -> 重建换 Arc）；与 SpawnDeps 共享同一 RwLock 句柄
    pub mrm: std::sync::Arc<std::sync::RwLock<std::sync::Arc<kxen_core::llm::mrm::ModelResourceManager>>>,
    pub extras: std::sync::Arc<kxen_core::agent::agent_loop::SessionExtrasRegistry>,
    /// Ask 档审批 broker（exec 高危命令的用户决定路由）
    pub approvals: std::sync::Arc<kxen_core::agent::approval::ApprovalBroker>,
    /// MCP、LSP 和 Hooks 按规范化 Workspace 根隔离，后台 Session 不跟随前台目录漂移。
    pub workspace_runtimes: std::sync::Arc<kxen_core::workspace_runtime::WorkspaceRuntimeRegistry>,
    pub team: std::sync::Arc<kxen_core::agent::team::TeamManager>,
    pub agents: std::sync::Arc<kxen_core::agent::activity::AgentRegistry>,
    /// session_id -> 进行中 run 的取消令牌（session.abort 用；run 结束自行移除）
    pub active_runs: std::sync::Mutex<std::collections::HashMap<String, kxen_core::agent::cancel::CancelToken>>,
    /// session 排队消息（落盘持久化：崩溃重启可恢复续跑；run 结束按序接续，防并发 run 交叉写历史）
    pub pending_messages: std::sync::Arc<kxen_core::core::pending_queue::PendingQueues>,
    /// session_id -> 已知 token 下界与 UNKNOWN 调用数（状态栏用量段）
    pub session_tokens: Arc<std::sync::Mutex<std::collections::HashMap<String, kxen_core::core::usage::SessionUsage>>>,
...
impl AppState {
    pub fn new() -> Result<Self, String> {
        ...
        for warning in kxen_core::core::usage::reconcile_provider_attempts(&mut loaded_usage)
        ...
        let knowledge_metering_operations = kxen_core::knowledge::consolidate::pending_metering_operation_ids()
            .map_err(|error| format!("Knowledge metering recovery scan failed: {error}"))?;
        for warning in
            kxen_core::core::usage::compact_closed_metering_receipts_preserving(&mut loaded_usage, &knowledge_metering_operations)
    ...
    pub fn extras_for(&self, session_id: &str) -> std::sync::Arc<kxen_core::agent::agent_loop::SessionExtras> {
    ...
    pub fn active_runtime(&self) -> Result<std::sync::Arc<kxen_core::workspace_runtime::WorkspaceRuntime>, String> {
    ...
    pub async fn ready_active_runtime(&self) -> Result<std::sync::Arc<kxen_core::workspace_runtime::WorkspaceRuntime>, String> {
        let root = self.active_workspace.read().map_err(|_| "workspace lock poisoned".to_string())?.clone();
