...
use kxen_core::agent::team::{SpawnDeps, TeamManager};
...
fn cron_jobs_of_deleted_session_never_fire() {
    ...
    assert_eq!(kxen_core::core::schedule::remove_by_session("ses_dead").unwrap(), 1);
    assert_eq!(kxen_core::core::schedule::remove_by_session("ses_dead").unwrap(), 0, "重复清理应幂等");
    // 存储层断言：已删 session 的 job 不在列表即永不再被 tick 出列（drain_due 只遍历内存 job 表）
    let jobs = kxen_core::core::schedule::list().unwrap();
    assert!(jobs.iter().all(|j| j.session_id != "ses_dead"), "已删 session 的 job 必须清除");
    assert!(jobs.iter().any(|j| j.id == b.id));
    kxen_core::core::schedule::remove(&a.id).unwrap();
    kxen_core::core::schedule::remove(&b.id).unwrap();
}
...
fn team_deps(fallback: &Path) -> SpawnDeps {
    SpawnDeps {
        ...
        runtimes: Arc::new(kxen_core::workspace_runtime::WorkspaceRuntimeRegistry::default()),
        extras: Arc::new(kxen_core::agent::agent_loop::SessionExtrasRegistry::default()),
...
fn team_dir_is_removed_on_session_delete() {
    ...
    let session = kxen_core::core::session::create(&sessions, root.to_str().unwrap()).unwrap();
    ...
    assert!(mgr.list_json(&session.id).unwrap().is_object());
    assert!(teams.join(&session.id).is_dir());
    mgr.drop_session(&session.id).unwrap();
    assert!(!teams.join(&session.id).exists(), "session 删除后 team 目录必须清掉");
    // 幂等与非法 id：都不许炸
    mgr.drop_session(&session.id).unwrap();
    assert!(mgr.drop_session("../escape").is_err());
...
fn fork_regenerates_message_ids_keeping_order_and_time() {
    ...
    for (i, fm) in forked_msgs.iter().enumerate() {
        ...
        assert!(kxen_core::core::ids::is_valid_id(&fm.id));
