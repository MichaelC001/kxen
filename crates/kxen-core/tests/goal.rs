// goal 生命周期 / 预算 / 证据校验测试。
use kxen_core::core::goal::{Goal, GoalBudget, GoalContract, GoalStatus, checked_turn_budget, evidence_sufficient};
...
fn loop_stop_reasons_escalate_to_blocked() {
    ...
    use kxen_core::agent::loop_detect::{LoopDetector, LoopVerdict};
...
async fn goal_tool_rejects_budget_limited_resume_and_accepts_adjust() {
    use kxen_core::agent::agent_loop::execute_goal_tool;
...
fn adjust_acknowledges_unknown_usage_without_losing_audit_count() {
    ...
    assert!(matches!(g.runtime_budget(kxen_core::core::shared::now_ms()), kxen_core::core::goal::RuntimeBudget::Unbounded));
}
...
fn explicit_auxiliary_usage_charges_the_requested_goal() {
    ...
    kxen_core::agent::agent_loop::charge_goal_usage_for("goal-charge-a", Some(17), None).unwrap();
