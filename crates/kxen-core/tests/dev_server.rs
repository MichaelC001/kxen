...
use kxen_core::tools::dev_server::{DevServerParams, ReadySpec, dev_server, restart_task};
use kxen_core::tools::task::{TaskOwner, TaskRegistry};
use std::sync::Arc;
...
async fn restart_keeps_id_and_ready_spec() {
    ...
    assert_eq!(info.status, kxen_core::tools::task::TaskStatus::Running, "重启就绪后应为 Running");
