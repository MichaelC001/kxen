...
use kxen_core::core::pending_queue::{PendingQueues, file_path};
...
fn ctx_file(path: &str) -> kxen_core::agent::context::ContextItem {
    kxen_core::agent::context::ContextItem::File { path: path.into() }
}

fn img() -> kxen_core::llm::types::ImagePart {
    kxen_core::llm::types::ImagePart { media_type: "image/png".into(), data: "aGVsbG8=".into() }
}
...
fn enqueue_claim_and_ack_persist_each_state() {
    ...
    assert!(matches!(first.context.first(), Some(kxen_core::agent::context::ContextItem::File { path }) if path == "a.rs"));
...
fn crash_after_session_append_replays_and_acknowledges_exactly_once() {
    ...
    kxen_core::core::session::append_message_idempotent_durable(&dir, &replayed).unwrap();
...
fn external_commit_failure_rolls_back_durable_queue_item() {
    ...
    let queue = PendingQueues::new(dir.clone());
    let item = kxen_core::core::pending_queue::QueuedMessage {
