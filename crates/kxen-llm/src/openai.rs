...
use crate::types::{Delta, Message, Role};
...
pub struct OpenAiProvider {
    http: reqwest::Client,
    bearer: kxen_core::shared::SharedStr,
    account_id: Option<kxen_core::shared::SharedStr>,
    subscription: bool,
}
...
impl OpenAiProvider {
    pub fn new(bearer: impl Into<String>, account_id: Option<String>, subscription: bool) -> Self {
        Self {
            http: crate::client::shared_http(),
            bearer: kxen_core::shared::SharedStr::from(bearer.into()),
            account_id: account_id.map(kxen_core::shared::SharedStr::from),
            subscription,
        }
    }

    pub fn stream_chat(&self, model: &str, messages: &[Message], tools: &[crate::tool::ToolDefinition]) -> Pin<Box<dyn futures::Stream<Item = Delta> + Send>> {
        ...
        let tools_owned: Vec<crate::tool::ToolDefinition> = tools.to_vec();
