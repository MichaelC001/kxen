...
use kxen_app::tools::dev_server::{dev_server, restart_task, DevServerParams, ReadySpec};
...
async fn main() {
    ...
    // restart
    let new_id = restart_task(&started.task_id, &registry).await.unwrap();
    ...
    let out = registry.output(&new_id);
