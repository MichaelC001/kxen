...
| 平台    | 签名机制                                                               |
| ------- | ---------------------------------------------------------------------- |
| macOS   | Apple Developer ID 签名 + Apple 公证，桌面 App、DMG、`kxen` 和 `kxen-agent` 一致 |
...
- macOS: `codesign --verify --deep --strict Kxen.app` 与 `xcrun stapler validate Kxen.app` 校验桌面 App；headless 包中的 `kxen` 和 `kxen-agent` 分别用 `codesign --verify --strict BINARY` 校验。
