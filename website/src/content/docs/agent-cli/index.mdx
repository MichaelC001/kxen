...
provider API key 永远不能通过 `--pass-env` 暴露给工具子进程。对于 Issue 自动修复，推荐把 GitHub read、repository edit、independent review 和 host publish 拆成不同进程，不把 GitHub token 交给通用 Shell fixer。
