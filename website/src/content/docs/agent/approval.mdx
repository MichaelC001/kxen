...
BotRun Cancel 同样先记录 durable cancellation request，再结束等待。迟到的普通 success 或 failure 不能覆盖取消；已经 Started 但结果无法确认的副作用进入 `UNKNOWN` 和 [Bot Recovery](https://kxen.ai/bots/runs-and-recovery)。
