...
Bot capability、Workspace path 和 Connector grant 在 publish 前由 Owner 明确授予。Builder Agent、Bot Message、CollaborationTask、Memory 和 ArtifactRef 都不能扩大权限。MCP tool 还需要 exact server grant 和 Connector 的 Workspace binding；Bot 当前不暴露无法按 path grant 完整过滤的 LSP。
