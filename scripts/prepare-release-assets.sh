...
# 用法: prepare-release-assets.sh <platform> <tag> <owner/repo> <bundle_root> [output_dir] [all|desktop|headless]
# platform 取值见 scripts/release-manifest.sh(平台/命名单一出处)。
# desktop 收集 Tauri bundle,headless 分别打包 kxen 与 kxen-agent,all 保持本地发布的完整单平台路径。
...
if [[ "$component" != headless && ! -d "$bundle_root" ]]; then
...
bash "$script_dir/verify-release-assets.sh" "$release_tag" "$repository" "$output_dir" --component "$component" "$platform"
printf 'PASS prepared release assets: %s (%s, %s)\n' "$output_dir" "$platform" "$component"
