...
# 用法: verify-release-assets.sh <tag> <owner/repo> [asset_dir] [--component all|desktop|headless] [platform...]
...
arguments=("${@:4}")
component=all
if [[ "${arguments[0]:-}" == --component ]]; then
  component="${arguments[1]:-}"
  platforms=("${arguments[@]:2}")
else
  platforms=("${arguments[@]}")
fi
...
case "$component" in
  all | desktop | headless) ;;
  *)
    printf 'release asset component must be all, desktop, or headless: %s\n' "$component"
    exit 1
    ;;
esac
if [[ ! -d "$asset_dir" ]]; then
...
if [[ "${#platforms[@]}" -eq 0 ]]; then
  if [[ "$component" != all ]]; then
    printf 'component subset verification requires at least one platform\n'
    exit 1
  fi
  full_set=1
...
expected=()
asset_function="kxen_release_${component}_assets"
if [[ "$component" == all ]]; then
  asset_function=kxen_release_assets
fi
for platform in "${platforms[@]}"; do
  while IFS= read -r name; do
    expected+=("$name")
  done < <("$asset_function" "$platform")
done
...
# 逐平台核对桌面 updater 与 headless package。component 子集用于并行 build job 的局部闭环,
# publish 仍以 all 模式重新核对合并后的完整文件集。
...
for platform in "${platforms[@]}"; do
  ...
  if [[ "$component" == all || "$component" == headless ]]; then
...
printf 'PASS release asset set: %s (%s files, %s)\n' "$asset_dir" "${#actual[@]}" "$component"
